...
// values. This version will print the output to a text file
// called WageOutput.txt
//
...
// note how this printEmployee function accepts the "ofstream &outputFile" parameter
// that was passed from the main function: e[i].printEmployee(outputFile, e[i]);
void Employee::printEmployee(ofstream &outputFile, Employee e) {

    // Display the entered input on the Employee
    // Also note that cout << is able to be replaced by outputFile << 
    // because outputFile was passed to this printEmployee function
    outputFile <<"\n\n *** Entered Details are *** \n";
...
int main ()
{
    ...
    // set up printing to a file called "WageOutput.txt"
    ofstream outputFile;
    outputFile.open("WageOutput.txt");

    // prompt for information to read in employee information
    for (int i = 0; i < EMP_SIZE; ++i)
    {
        // Enter Employee Information in the console, to be printed in file
